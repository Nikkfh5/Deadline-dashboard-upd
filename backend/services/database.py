...
async def init_db() -> AsyncIOMotorDatabase:
    ...
    await _db.folders.create_index("id", unique=True)
    await _db.notes.create_index([("user_id", 1), ("folder_id", 1), ("order", 1)])
    await _db.notes.create_index("id", unique=True)
    await _db.deadlines.create_index([("user_id", 1), ("folder_id", 1), ("due_date", 1)])
